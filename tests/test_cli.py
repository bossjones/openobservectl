...
def _server(httpserver: HTTPServer, *, streams: list[dict[str, Any]] | None = None) -> str:
...
def test_health_reports_status(httpserver: HTTPServer) -> None:
...
def test_streams_lists_names(httpserver: HTTPServer) -> None:
...
def test_streams_sends_basic_auth(httpserver: HTTPServer) -> None:
    httpserver.expect_request("/healthz").respond_with_json({"status": "ok"})
...
def test_streams_surfaces_error_body_when_response_has_no_list_key(
...
def test_search_returns_hits(httpserver: HTTPServer) -> None:
    base = _server(httpserver)
...
def test_query_promql(httpserver: HTTPServer) -> None:
    base = _server(httpserver)
...
def test_logs_tail_without_follow_single_window(httpserver: HTTPServer) -> None:
    base = _server(httpserver)
    httpserver.expect_request("/api/default/_search", method="POST").respond_with_json(
...
def test_logs_tail_autodiscovers_logs_streams(httpserver: HTTPServer) -> None:
...
def test_logs_tail_no_logs_streams_exits_nonzero(httpserver: HTTPServer) -> None:
...
def test_logs_tail_stream_and_sql_together_exits_nonzero(httpserver: HTTPServer) -> None:
...
def test_logs_tail_sql_override_skips_autodiscovery(httpserver: HTTPServer) -> None:
...
def test_logs_tail_json_flag_emits_compact_json_lines_only(httpserver: HTTPServer) -> None:
    base = _server(httpserver, streams=[{"name": "container_logs", "stream_type": "logs"}])
    httpserver.expect_request("/api/default/_search", method="POST").respond_with_json(
...
def test_logs_tail_invalid_since_exits_nonzero(httpserver: HTTPServer) -> None:
    base = _server(httpserver)
...
def test_check_passes_when_healthy_and_authed(httpserver: HTTPServer) -> None:
    base = _server(httpserver)
...
def test_check_fails_on_auth_401(httpserver: HTTPServer) -> None:
    httpserver.expect_request("/healthz").respond_with_json({"status": "ok"})
...
def test_check_streams_present_is_skip_by_default_when_empty(httpserver: HTTPServer) -> None:
    base = _server(httpserver, streams=[])
    r = _run(base, "--json", "check")
    assert r.exit_code == 0, r.output


def test_check_require_streams_fails_when_empty(httpserver: HTTPServer) -> None:
    base = _server(httpserver, streams=[])
...
def test_check_fails_on_connection_refused() -> None:
...
def test_check_require_metrics_passes_when_up_returns_series(httpserver: HTTPServer) -> None:
    base = _server(httpserver)
    httpserver.expect_request("/api/default/prometheus/api/v1/query").respond_with_json(
        {
            "status": "success",
...
def test_check_require_metrics_fails_when_no_series(httpserver: HTTPServer) -> None:
    base = _server(httpserver)
    httpserver.expect_request("/api/default/prometheus/api/v1/query").respond_with_json(
...
def test_check_require_logs_passes_when_stream_has_rows(httpserver: HTTPServer) -> None:
    base = _server(httpserver, streams=[{"name": "container_logs", "stream_type": "logs"}])
    httpserver.expect_request("/api/default/_search", method="POST").respond_with_json(
...
def test_check_require_logs_fails_when_no_logs_streams(httpserver: HTTPServer) -> None:
...
def test_server_url_flag_beats_env_and_profile(
...
def test_env_url_beats_profile(
...
def test_profile_endpoint_used_when_no_flag_or_env(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, httpserver: HTTPServer
) -> None:
    base = _server(httpserver)
    cfg = tmp_path / "config.yaml"
...
def test_no_source_exits_nonzero_with_guidance(
