...
def test_load_config_parses_profiles(sample_config_path):
...
def test_profile_optional_fields_parsed(sample_config_path):
    cfg = load_config(sample_config_path)
...
def test_endpoint_trailing_slash_normalized(sample_config_path):
    cfg = load_config(sample_config_path)
...
def test_resolve_profile_returns_named(sample_config_path):
    cfg = load_config(sample_config_path)
    prof = resolve_profile(cfg, "dev", env={})
    assert prof.organization == "dev-org"


def test_resolve_profile_unknown_lists_available(sample_config_path):
    cfg = load_config(sample_config_path)
    with pytest.raises(ConfigError) as exc:
...
def test_env_overrides_win_over_profile(sample_config_path):
    cfg = load_config(sample_config_path)
    env = {
...
def test_env_override_endpoint_only_keeps_other_fields(sample_config_path):
    cfg = load_config(sample_config_path)
...
def test_env_override_endpoint_trailing_slash_normalized(sample_config_path):
    cfg = load_config(sample_config_path)
    prof = resolve_profile(cfg, "default", env={"OPENOBSERVE_URL": "http://10.0.0.9:5080/"})
    assert prof.endpoint == "http://10.0.0.9:5080"
