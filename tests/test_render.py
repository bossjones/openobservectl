...
def test_format_timestamp_micros_to_iso_utc() -> None:
    # 1970-01-01T00:00:01+00:00 == 1_000_000 microseconds
    assert format_timestamp(1_000_000).startswith("1970-01-01T00:00:01")


def test_format_timestamp_zero_or_missing_is_dash() -> None:
    assert format_timestamp(0) == "-"


def test_render_hit_json_is_single_line_and_roundtrips() -> None:
...
def test_render_hit_pretty_contains_ts_level_and_message() -> None:
...
def test_render_hit_prefers_message_then_log_then_body() -> None:
...
def test_render_hit_without_known_message_dumps_remaining_fields() -> None:
...
def test_render_hit_tolerates_missing_timestamp() -> None:
    out = render_hit({"log": "no ts here"}, as_json=False)
    assert out.startswith("-")
    assert "no ts here" in out


def test_write_hit_json_mode_never_wraps_long_lines() -> None:
